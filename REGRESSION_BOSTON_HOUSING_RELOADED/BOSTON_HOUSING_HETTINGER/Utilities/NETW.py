...
class Final(tf.keras.Model):
    def __init__(self, hyperp, run_options, data_input_shape, output_dimensions, kernel_regularizer, bias_regularizer,trainable):
...
###############################################################################
        ...
        self.class_act=hyperp.classification_act
...
###############################################################################     
    ...
    def add_layer(self, trainable_hidden_layer_index, freeze=True ,add = True):
        ...
        del self.classification_layer
        self.classification_layer = Dense(units = self.output_dim,
                                          activation = self.class_act, use_bias = True, kernel_initializer = kernel_initializer, bias_initializer = bias_initializer,                              
