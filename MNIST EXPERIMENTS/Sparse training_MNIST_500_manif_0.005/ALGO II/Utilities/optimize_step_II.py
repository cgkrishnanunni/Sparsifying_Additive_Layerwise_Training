...
#                             Training Properties                             #
###############################################################################
def optimize_step(x_train,y_train,data_test, labels_test,i_val,label_dimensions,hyperp,hyperp_new,run_options,data_input_shape,accuracy,labels_train):
    #=== Optimizer ===#
    ...
    model.add(layers.Dense(500, activation='elu', input_shape=(784,)))
    ...
    model.fit(x_train,y_train,batch_size=900,epochs=400,verbose=1)
    
    batch_pred_test = model(data_test)

    #batch_pred_test = model(x_train)
    ...
    #mean_accuracy_test(accuracy(batch_pred_test, labels_train))
