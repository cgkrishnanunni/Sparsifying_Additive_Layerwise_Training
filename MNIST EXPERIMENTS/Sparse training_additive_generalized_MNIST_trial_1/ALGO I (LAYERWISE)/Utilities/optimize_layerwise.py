...
#                             Training Properties                             #
###############################################################################
def optimize(hyperp, hyperp_new,run_options, file_paths, NN, data_loss, accuracy, data_and_labels_train, data_and_labels_val, data_and_labels_test, label_dimensions, num_batches_train,data_and_labels_train_new,manifold_class,batch_size,random_seed,num_data_train,i_val,data_input_shape):
    #=== Optimizer ===#
...
############################################################################### 
    ...
    while trainable_hidden_layer_index < hyperp_new.max_hidden_layers:  
        ...
        for epoch in range(hyperp_new.num_epochs):
            ...
            lrate = 0.001 * math.pow(0.95,(epoch))  
            ...
            for batch_num, (batch_data_train, batch_labels_train,labels) in data_and_labels_train_new_new.enumerate():
                with tf.GradientTape() as tape:
                    ...
                    batch_loss_train +=fr* sum(NN.losses)+ fr*manifold_class(batch_data_train, batch_labels_train, NN,manifold_regul,label_dimensions)
            ...
            for batch_num_num, (batch_data_val, batch_labels_val,labels_val) in data_and_labels_val.enumerate():
                batch_pred_val,val_new = NN(batch_data_val)
                
            
                y_pred_val_add=net_output(hyperp,hyperp_new,batch_data_val, run_options, data_input_shape, label_dimensions,i_val,batch_pred_val)
        
                batch_pred_val=batch_pred_val+y_pred_val_add
                #batch_pred_test=y_pred_test_add
                
                mean_loss_val(data_loss(batch_pred_val, labels_val, label_dimensions,i_val))
        ...
        if trainable_hidden_layer_index < hyperp_new.max_hidden_layers:
