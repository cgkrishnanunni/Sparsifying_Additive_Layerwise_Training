...
############################################################################### 
    ...
    model_constraint=hyperp_new.model_constraint
    ...
    while trainable_hidden_layer_index < hyperp_new.max_hidden_layers:  
        ...
        for epoch in range(hyperp_new.num_epochs):
            ...
            #lrate=np.random.uniform(low=0.0005, high=0.005)
            #if epoch>1000:
                #lrate = 0.001 * math.pow(1,(epoch)) 
            #lrate=np.random.uniform(low=0.0001, high=0.01)
            #if epoch>200:
             #   lrate=0.001 * math.pow(1,(epoch))  
            ...
            for batch_num, (batch_data_train, batch_labels_train,labels) in data_and_labels_train_new_new.enumerate():
                with tf.GradientTape() as tape:
                    ...
                    batch_loss_train = data_loss(batch_pred_train, labels, label_dimensions,i_val)+compute_interior_loss(gauss_points_new, batch_labels_train, NN,model_constraint,label_dimensions,hyperp,hyperp_new,data_input_shape,i_val,batch_pred_train,run_options,gauss_weights_new,Coordinates, Stiffness, load,Solution,v)
            ...
            #if mean_accuracy_test.result()<0.004:
                #break
