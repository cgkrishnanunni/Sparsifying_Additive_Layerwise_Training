...
def plot_sine(hyperp,hyperp_new, data, run_options, data_input_shape, label_dimensions,i_val,X_train, Y_train,labels_test):
    ...
    fig, ax = plt.subplots()
    clevels = np.linspace(np.amin(y_pred_test_add-labels_test), np.amax(y_pred_test_add-labels_test), 10000)
    #v = np.linspace(0, 8, 10, endpoint=True)
    im = ax.tricontourf(x, y, y_pred_test_add[:,0]-labels_test[:,0], clevels, cmap='RdYlBu_r')
