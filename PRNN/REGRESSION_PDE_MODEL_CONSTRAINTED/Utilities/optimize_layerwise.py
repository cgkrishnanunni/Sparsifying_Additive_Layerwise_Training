...
def optimize(hyperp, hyperp_new,run_options, file_paths, NN, data_loss, accuracy, data_and_labels_train, data_and_labels_val, data_and_labels_test, label_dimensions, num_batches_train,data_and_labels_train_new,manifold_class,batch_size,random_seed,num_data_train,i_val,data_input_shape,data_train,labels_train,multiply,trainable_hidden_layer_index,compute_interior_loss,gauss_points,gauss_weights,error_L2,gauss_solution,gauss_points_new,gauss_weights_new,Coordinates, Stiffness, load,Solution,v,data_train_inside, labels_train_inside):
    ...
    mean_loss_on_data=tf.keras.metrics.Mean()
...
############################################################################### 
    ...
    while trainable_hidden_layer_index < max_hidden_layers: 
        ...
        for epoch in range(hyperp_new.num_epochs):
            ...
            for batch_num, (batch_data_train, batch_labels_train,labels) in data_and_labels_train_new_new.enumerate():
                with tf.GradientTape() as tape:
                    ...
                    batch_loss_train += sum(NN.losses)+compute_interior_loss(gauss_points_new, batch_labels_train, NN,model_constraint,label_dimensions,hyperp,hyperp_new,data_input_shape,i_val,batch_pred_train,run_options,gauss_weights_new,Coordinates, Stiffness, load,Solution,v)+manifold_class(data_train_inside, labels_train_inside, NN,manifold_regul,label_dimensions)
                ...
                mean_loss_train(batch_loss_train) 
                mean_loss_on_data(data_loss(batch_pred_train, labels, label_dimensions,i_val))
                ...
                mean_accuracy_val(sum(NN.losses))
                mean_loss_val(manifold_class(data_train_inside, labels_train_inside, NN,manifold_regul,label_dimensions))
            ...
            storage_array_loss = np.append(storage_array_loss, (mean_loss_train_constraint.result())/model_constraint+mean_loss_on_data.result())
            
            storage_array_accuracy = np.append(storage_array_accuracy, mean_loss_train.result())
            ...
            start_time_epoch = time.time()   
            rr=mean_loss_on_data.result()
            bb=mean_loss_train_constraint.result()/model_constraint
            jj= mean_accuracy_test.result()
            ...
            mean_loss_on_data.reset_states()
        ...
        print('Training Set data: Loss: %.3e, AConstraint: %.3f' %(rr, bb))

        print('Test Set: Loss L2: %.3e, Test data loss: %.3f\n' %(L2_error, jj))
