...
def load_data(NN_type, dataset, random_seed):
    ...
    if dataset == 'Abalone':
        ...
        data_train=np.loadtxt("data_train_total.data")
        labels_train=np.loadtxt("labels_train_total.data")
        data_test=np.loadtxt("data_test.data")
        labels_test=np.loadtxt("labels_test.data")
        
        data_train_inside=np.loadtxt("data_train_inside.data")
        labels_train_inside=np.loadtxt("labels_train_inside.data")
    ...
    data_train_inside = tf.cast(data_train_inside,tf.float32)
    labels_train_inside = tf.cast(labels_train_inside, tf.float32)
    ...
    return data_train, labels_train, data_test, labels_test, data_input_shape, num_channels, label_dimensions, data_train_inside, labels_train_inside