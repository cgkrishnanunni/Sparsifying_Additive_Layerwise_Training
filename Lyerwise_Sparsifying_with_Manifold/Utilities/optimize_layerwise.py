...
#                             Training Properties                             #
###############################################################################
def optimize(hyperp, run_options, file_paths, NN, data_loss, accuracy, data_and_labels_train, data_and_labels_val, data_and_labels_test, label_dimensions, num_batches_train,data_and_labels_train_new,manifold_class,batch_size,random_seed,num_data_train):
    #=== Optimizer ===#
...
############################################################################### 
    ...
    manifold_regul=hyperp.manifold
    ...
    while loss_validation > hyperp.error_TOL and trainable_hidden_layer_index < hyperp.max_hidden_layers:  
        ...
        if trainable_hidden_layer_index>2:
            manifold_regul=manifold_regul/(1+(trainable_hidden_layer_index-2))
        ...
        for batch_data_val, batch_labels_val in data_and_labels_val:
            batch_pred_val,val = NN(batch_data_val)
        ...
        for epoch in range(hyperp.num_epochs):
            ...
            for batch_num, (batch_data_train, batch_labels_train) in data_and_labels_train_new_new.enumerate():
                with tf.GradientTape() as tape:
                    ...
                    batch_loss_train_new_one= data_loss(batch_pred_train, batch_labels_train, label_dimensions)
    ...
    #NN.save_weights(file_paths.NN_savefile_name+ "model_weights")
